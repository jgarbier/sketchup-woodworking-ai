...
AI agent (MCP client) → official TypeScript MCP SDK → localhost JSON bridge → Ruby extension → SketchUp API.
...
Project YAML records design intent; SketchUp components carry stable project/part IDs.
One logical agent edit is one undo operation. Exports derive from structured parts.

## Current implementation

The JSON bridge and MCP server are fully implemented. The extension loads on SketchUp startup
and starts the bridge automatically. UI.start_timer polls nonblocking TCP sockets on the
SketchUp main thread. There is no worker-thread access to the SketchUp API.
...
This local endpoint trusts local OS processes; there is no network authentication token.
