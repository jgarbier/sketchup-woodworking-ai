...
%w[geometry project_validation projects project_observer scenes reports exporter command_router].each { |name| require File.join(root, name) }
%w[geometry project_validation projects project_observer scenes reports exporter command_router].each { |name| load File.join(root, "#{name}.rb") }
